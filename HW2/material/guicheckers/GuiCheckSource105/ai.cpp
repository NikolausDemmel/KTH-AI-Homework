...
int QuiesceBoard( int color, int ahead, int alpha, int beta )
	...
	if (g_Movelist[ahead].nJumps == 0 || (ahead+1) >= MAX_SEARCHDEPTH) 
		{
		if (ahead > g_SelectiveDepth) g_SelectiveDepth = ahead-1;
	...
	for (i = 0; i <= g_Movelist[ahead].nMoves; i++)
        ...
        //If this isn't the max depth continue to look ahead 
		  ...
		  if (ahead == 1 && abs(value)<1000)
			{
			 if ( Repetition( g_Boardlist[ahead].HashKey, 0, g_nMoves+1) ) value = (value>>1) + (value>>2);
...
int ComputerMove( char cColor, CBoard &InBoard)
	...
	if (cColor == BLACK) g_Movelist[1].FindMovesBlack( InBoard.Sqs, InBoard.C );
	if (cColor == WHITE) g_Movelist[1].FindMovesWhite( InBoard.Sqs, InBoard.C );
	if (g_Movelist[1].nMoves == 0) return 0; // game over
			 ...
			 if (Eval!=TIMEOUT) 
				{
				 ...
				 if (g_Movelist[1].nMoves == 1 && g_MaxDepth > 6) Eval = TIMEOUT;
