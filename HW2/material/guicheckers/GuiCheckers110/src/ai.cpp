...
void inline DoExtensions( int &nextDepth, int &fracDepth, int ahead, unsigned long nMove )
{
	...
	if (g_Movelist[ahead].numJumps == 1) 	{ 		// Single move
		if (ahead>1 && g_Movelist[ahead-1].numJumps!=0) fracDepth+=22; // after opponent jump
		else if (ahead >2 && g_Movelist[ahead-2].numJumps == 1) fracDepth+=17; // we had to jump last move, now this move to
		else fracDepth+=8;
	}
	// Extend if the checker moves close to promotion (2 moves) 
	else if ( S[dst]&MASK_6TH && g_Boardlist[ahead].GetPiece(dst) == BPIECE && g_Boardlist[ahead].GetPiece(dst+8) == EMPTY ) fracDepth+=17;
	else if ( S[dst]&MASK_3RD && g_Boardlist[ahead].GetPiece(dst) == WPIECE && g_Boardlist[ahead].GetPiece(dst-8) == EMPTY ) fracDepth+=17;
	...
	else if ( (dst > 23 && g_Boardlist[ahead].GetPiece(dst) == BPIECE ) || (dst < 8 && g_Boardlist[ahead].GetPiece(dst) == WPIECE )) fracDepth+=8;
...
short ABSearch(int color, int ahead, int depth, int fracDepth, short alpha, short beta, int &bestmove)
{
	...
	if (nodes > nodes2 + 50000 ) 
	{
		if ( bCheckerBoard && *g_pbPlayNow ) return TIMEOUT;
		if ( g_bStopThinking ) return TIMEOUT;
		endtime = clock();
		nodes2 = nodes;
	...
	for (i = 0; i <= g_Movelist[ahead].numMoves; i++)
        ...
        //If this isn't the max depth continue to look ahead 
				...
				{
					int bForced = false;
					if (ahead > 1 && g_Movelist[ahead-1].numMoves ==1 && g_Movelist[ahead].numMoves ==1) bForced = true;
					...
					{
						if ( g_Boardlist[ahead].eval == 0 || (g_dbInfo.type == DB_EXACT_VALUES && abs(g_Boardlist[ahead].eval) > 1800) )
		  ...
		  if (ahead == 1 && abs(value)<1000)
			{
			 if ( Repetition( g_Boardlist[ahead].HashKey, 0, g_numMoves+1) ) value = (value>>1) + (value>>2);
...
int ComputerMove( char cColor, CBoard &InBoard)
{
	...
	if (InBoard.C.BP == 0 || InBoard.C.WP == 0) return 0;
	if (cColor == BLACK) g_Movelist[1].FindMovesBlack( InBoard.C );
	if (cColor == WHITE) g_Movelist[1].FindMovesWhite( InBoard.C );
	if (g_Movelist[1].numMoves == 0) return 0; // game over
	...
	if ( g_SearchEval == NOVAL ) 
		...
		if (!bCheckerBoard) ReplayGame( g_numMoves, InBoard); 
			 ...
			 if (Eval!=TIMEOUT) 
				{
				 ...
				 if (g_Movelist[1].numMoves == 1 && g_MaxDepth > 6) Eval = TIMEOUT;
	...
	if ( (clock ()-starttime) < (CLOCKS_PER_SEC/4) && !bCheckerBoard) Sleep(200); // pause a bit if move is really quick
