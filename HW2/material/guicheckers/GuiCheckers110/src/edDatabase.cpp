...
int inline AddPieces( unsigned char piece[6], int &piece_count, UINT PieceBits )
{
	int start_count = piece_count;
	while ( PieceBits ) 
	{
		UINT sq = FindLowBit( PieceBits );
		PieceBits &= ~S[sq];
		
		piece[ piece_count++ ] = (unsigned char)(FlipX(sq)+1);
	}
...
int inline AddPiecesFlipped( unsigned char piece[6], int &piece_count, UINT PieceBits )
{
	int start_count = piece_count;
	while ( PieceBits ) 
	{
		UINT sq = FindHighBit( PieceBits );
		PieceBits &= ~S[sq];
		
		piece[ piece_count++ ] = (unsigned char)(32-FlipX(sq));
	}
	int numType = piece_count - start_count;
...
int QueryEdsDatabase( const CBoard &Board, int ahead )
{
	...
	{
		if ( Board.nWhite >= Board.nBlack )
		{
			white_king_count = AddPieces ( piece, piece_count, Board.C.WP & Board.C.K );
			white_checker_count = AddPieces ( piece, piece_count, Board.C.WP & ~Board.C.K );
			red_king_count = AddPieces ( piece, piece_count, Board.C.BP & Board.C.K );
			red_checker_count = AddPieces ( piece, piece_count, Board.C.BP & ~Board.C.K );
		}
		else
		{
			red_king_count = AddPieces ( piece, piece_count, Board.C.BP & Board.C.K );
	...
	else
	{
		...
		if ( Board.nWhite > Board.nBlack )
		{
			red_king_count = AddPiecesFlipped ( piece, piece_count, Board.C.WP & Board.C.K );
			red_checker_count = AddPiecesFlipped ( piece, piece_count, Board.C.WP & ~Board.C.K );
			white_king_count = AddPiecesFlipped ( piece, piece_count, Board.C.BP & Board.C.K );
			white_checker_count = AddPiecesFlipped ( piece, piece_count, Board.C.BP & ~Board.C.K );
		}
		else
		{
			white_king_count = AddPiecesFlipped ( piece, piece_count, Board.C.BP & Board.C.K );
	...
	assert( Board.nWhite + Board.nBlack == white_king_count + white_checker_count + red_king_count + red_checker_count );

	which_db = Board.nWhite + Board.nBlack;
