...
int color = BLACK;					/* color is the side to move next */
...
PROC6 islegal=0,islegal1=0,islegal2=0;
PROC3 enginename1=0,enginename2=0;
int enginename(char Lstr[256]);
int gametype(void);
BOOL fFreeResult;
PROC4 CBgametype=0; // built in gametype and islegal functions 
PROC6 CBislegal=0;
			...
			switch(LOWORD(wParam)) 
				...
				case SELECTUSERBOOK:
					...
					if(getfilename(userbookname,OF_USERBOOK))
						...
						sprintf(str,"found %i positions in user book", userbooknum);
						}
					break;
				...
				case MOVESBACK:	
					...
					if(CBstate == BOOKVIEW && userbooknum != 0)
						...
						sprintf(str,"position %i of %i: %i-%i", userbookcur+1,userbooknum,coortonumber(userbook[userbookcur].move.from, GPDNgame.gametype), coortonumber(userbook[userbookcur].move.to,GPDNgame.gametype));
				...
				case BOOKMODE_DELETE:
					...
					if(CBstate == BOOKVIEW && userbooknum!=0)
						...
						for(i=userbookcur;i<userbooknum-1;i++)
	...
	if(setup)
		...
		if( (x+y+1)%2)
			...
			switch(board8[x][y])
				{
				case WHITE|MAN: 
					board8[x][y] = WHITE|KING;
					break;
				case WHITE|KING: 
					board8[x][y] = 0;
					break;
				default: 
					board8[x][y] = WHITE|MAN;
					break;
				}
			}
		updateboardgraphics(hwnd);
...
int handle_lbuttondown(int x, int y)
	...
	if(setup)
		...
		if( (x+y+1)%2)
			...
			switch(board8[x][y])
				{
				case BLACK|MAN: 
					board8[x][y]=BLACK|KING;
					break;
				case BLACK|KING: 
					board8[x][y]=0;
					break;
				default: 
					board8[x][y]=BLACK|MAN;
					break;
				}
			}
		updateboardgraphics(hwnd);
	...
	if(x1==-1) //then its the first click
		...
		if(islegal != NULL)
			...
			for(i=1; i<=32; i++)
				{
				if(islegal((int *)board8, color, coorstonumber(x1,y1,GPDNgame.gametype), i, &GCBmove) != 0)
					{
					legal++;
					legalmovenumber = i;
		...
		if((color==BLACK && board8[x1][y1]&BLACK) || (color==WHITE && board8[x1][y1]&WHITE))
			{
		...
		if((color==BLACK && board8[x2][y2]&BLACK) || (color==WHITE && board8[x2][y2]&WHITE))
			{
			// re-print board to overwrite last selection if there was one
			updateboardgraphics(hwnd);
			// and then select stone
		...
		if(islegal!=NULL)
			{
			if(islegal((int *)board8,color,coorstonumber(x1,y1,GPDNgame.gametype),coorstonumber(x2,y2, GPDNgame.gametype),&GCBmove)!=0)
				// a legal move!
				{
				// insert move in the linked list 
				appendmovetolist(GCBmove);
...
int addmovetouserbook(int b[8][8], struct CBmove *move)
	...
	if(n == userbooknum)
		{
		(userbooknum)++;
		sprintf(str,"added move to userbook (%i moves)",userbooknum);
		}
	else
		sprintf(str,"replaced move in userbook (%i moves)",userbooknum);
	// save user book
	fp = fopen(userbookname,"wb");
	if(fp != NULL)
		{
	...
	if(how == RE_SEARCH)
		...
		if(strcmp(database,"")!=0 && result)
			...
			if(how == GAMEFIND || how == GAMEFINDTHEME || how == GAMEFINDCR ||
				...
				if(how == GAMEFINDCR)
					gamenumber = pdnfind(&currentposition, color^CC, gamelist, &r);
...
int start11man(int number)
	...
	FENtoboard8(board8, str, &color, 21);

	// set linked list 
	initlinkedlist();
	GPDNgame.head = head;
	board8toFEN(board8, str, color, 21);
...
// to a file if CB is in either ANALYZEGAME or ENGINEMATCH mode 
	...
	if(!founduserbookmove)
		...
		if(result!=UNKNOWN && (CBstate==ENGINEMATCH))
			{
		...
		if(CBstate!=OBSERVEGAME && CBstate!=ANALYZEGAME && CBstate!=ANALYZEPDN && !abortcalculation)
			...
			if(gametype()==21) /* should be 'if gametype==21' */
				{
				if(color==BLACK)
...
void pdntogame(int startposition[8][8], int startcolor)
	...
	while( current->next !=NULL)
		...
		if(islegal((int *)b8,col,from,to,&legal))
			{
			current->move=legal;
			domove(legal,b8);
			col=col^CHANGECOLOR;
...
// are loaded right now.
	...
	CBgametype = (PROC4)builtingametype;
	CBislegal = (PROC6)builtinislegal;
	...
	if (hinstLib1 != NULL)
		...
		if(loadok)
			{
			enginecommand1 = (COMMANDPROC) GetProcAddress(hinstLib1,"enginecommand");
			enginename1 =  (PROC3) GetProcAddress(hinstLib1, "enginename");
			getmove1 =  (PROC1)GetProcAddress(hinstLib1, "getmove");
			islegal1= (PROC6)GetProcAddress(hinstLib1,"islegal");			
			}
		else
			{
	...
	if (hinstLib2 != NULL)
		...
		if(loadok)
			{
			enginecommand2 =  (COMMANDPROC) GetProcAddress(hinstLib2, "enginecommand");
			enginename2 =  (PROC3) GetProcAddress(hinstLib2, "enginename");
			getmove2 =  (PROC1)GetProcAddress(hinstLib2, "getmove");
			islegal2= (PROC6)GetProcAddress(hinstLib2,"islegal");
			}
		else
			{
