package org.jscience.computing.hmm;
 ...
 * Forward-Backward, Viterbi, K-means, Baum-Welch, and Kullback-Leibler
 * distance measure.
 *
 * @author Kanav Kahol
...
/**
     * Class constructor. Initializes an HMM given number of states and output
     * vocabulary size.
     *
...
/**
     * Class constructor. Initializes an HMM a given number of states, output
     * vocabulary size, and number of dimensions for its observation symbols.
     *
     * @param numStates  number of states
     * @param sigmaSize  size of output vocabulary
     ...
     * observation sequence o, it will train this HMM using o to increase the
     * probability of this HMM generating o.
     *
    ...
    /**
     * Calculation of Forward variables f(i,t) for state i at time t
     * for sequence o with the current HMM parameters.
     *
     * @param o the observation sequence
     *
     * @return a 2d array containing Forward variables f(i,t) over states and
     *         time
     */
    ...
    /**
     * Calculation of Backward variables b(i,t) for state i at time t
     * for sequence o with the current HMM parameters.
     *
     * @param o the observation sequence
     *
     * @return a 2d array containing Backward variables b(i,t) over states and
     *         time
     */
    ...
    /**
     * Calculation of xi_t(i, j), which is the probability P(i_t = s_i,
     * i_t+1 = s_j | o, hmm), that is, the probability of being in state i at
     * time t and state j at time t+1 given observation sequence o and this
     * HMM.
     *
     * @param t the time
     ...
     */
    public double xi(int t, int i, int j, int[] o, double[][] fwd,
        double[][] bwd) {
        double num;
        double denom = 0.0;
    ...
    /**
     * Calculation of gamma_t(i), which is the probability P(i_t = s_i
     * | o, hmm), that is, the probability of being in state i at time given
     * observation sequence o and this HMM.
     *
     * @param i the number of state s_i
     * @param t the time
     * @param o the observation sequence
    ...
    /**
     * K-means algorithm for hidden Markov models. Given training data,
     * this algorithm will classify the data into a specified number of
     * states, then using the final classification, calculate initial,
     * transition, and bias probabilities.
     *
     ...
     */
    public static HMM kmeansLearner(String data, int N, int steps)
        throws IOException {
        HMM newHmm = null;
        ...
        while (!done) {
            ...
            for (int i = 0; i < N; i++) {
                ...
                for (int d = 0; d < D; d++)
                    RMS += ((means[i][d] - new_means[i][d]) * (means[i][d] -
                    new_means[i][d]));
     ...
     * algorithm.
     */
    private static void setInitialMeans(double[][] means,
        double[][][] trainingData, int D, int w, int T, int N) {
     ...
     */
    private static void probabilitiesFromKmeans(HMM newHmm,
        int[][] classification, double[][][] trainingData, int N, int D, int w,
        int T) {
            ...
            for (int j = 0; j < w; j++) {
                for (int t = 0; t < T; t++) {
                    if (classification[j][t] == i) {
                        ...
                        for (int b = 0; (b < newHmm.sigmaSize) && !found;
                                b++)
     ...
     */
    private static int getSymbols(double[][] symbolSet,
        double[][][] trainingData, int D, int w, int T) {
     ...
     * the Kullback-Leibler distance measure. The models should have the same
     * size vocabulary.
     *
     * @param hmm2 the hidden Markov model that will be compared against this
     *        HMM
     *
     * @return the symmetric distance between this HMM and hmm2 (if their
     *         vocabulary size differs, -1 is returned)
     */
    public double hmmDistance(HMM hmm2) {
        ...
        d12 = (1.0 / 4000.0) * Math.abs((-1 * weight11[0][0]) -
                (-1 * weight21[0][0]));
        d21 = (1.0 / 4000.0) * Math.abs((-1 * weight22[0][0]) -
                (-1 * weight12[0][0]));
    ...
    /**
     * Generates an observation sequence of a given length using this
     * HMM.
     *
    ...
    public int[] observationGeneration(int length) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            for (int j = 1; (j < sigmaSize) && !done; j++) {
                if ((randVal > total) &&
                        (randVal <= (total + b[currentState][j]))) {
                    o[i] = j;
                    done = true;
                }
            ...
            for (int j = 1; (j < numStates) && !done; j++) {
                if ((randVal > total) &&
                        (randVal <= (total + a[currentState][j]))) {
                    currentState = j;
                    done = true;
                }
     ...
     */
    private static double euclidDistance(double[] O, double[][] V, int dim,
        int k) {
        int i;
        double sum = 0;
     ...
     * indices based on the vocabulary of this HMM. Any time a hidden Markov
     * model algorithm whose input involves an observation sequence is used,
     * the observation sequence should be converted to indices if it's not
     * already.
     *
    ...
    /**
     * Prints all the parameters of this HMM. Parameters include
    ...
    /**
     * Loads an HMM from a file. A new HMM is created and the read
     * values are stored as the HMM parameters. Returned is the new HMM.
     *
     * @param filename name of file to read the HMM from
     *
     * @return HMM loaded from the given file
     *
     ...
     * number of states, output vocabulary size, number of dimensions (-1 if
     * not used), initials, transitions, biases, and, if it was used, the
     * output vocabulary.
     *
